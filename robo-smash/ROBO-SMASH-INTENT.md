...
- **Belly flop** (S or ↓ in air): 4f hang, then a straight slam faster than MAXFALL (no steering — commitment is the cost). Smashes through crates in the path, squishes scuttlers without bouncing, and lands with a shockwave (~1.6 tiles) that cracks everything nearby, lights TNT fuses (direct slam contact = instant detonation, Crash-style — it hurts), and dusts the screen. **Bolted crates (`R`, "▼" faceplate) open ONLY to the slam** — spin clanks off them, bounce treats them as plain platforms, explosions can't crack them. They always pay out (rich drop table) — the slam-only loot is the reason to learn the verb. Slam is cancelled by taking a hit; slamming into a pit is still a pit death.
...
- **Seed sharing:** fresh seeds mix timestamp + entropy per page load; `C` (or the button on the run-complete screen) copies a `?seed=N` link that replays the exact run — layout, knobs, and crate drops included.
...
- **Semi-3D (decided 2026-07-28, revisited and REJECTED same day): no depth axis, continuous or discrete.** Continuous z breaks the adversarial stack (deterministic 2D sim, jump-template solver, 1-D landing distribution). Discrete lanes were considered (2 planes, authored shift pads) but rejected on two grounds: (a) the two-plane rendering was judged unlikely to read at 960×540/48px tiles; (b) **the control surface has no room** — every comfortable key is spent on a shipped verb (A/D run, Space/W/↑ jump, J/K/Shift spin, S/↓ slam), so lanes would evict slam or force modal overloads (S shifts on ground, slams in air), which produce accidental wrong-verbs under pressure — a C1-adjacent sin. When a feature forces a full input redesign and still comes up a key short, it's fighting the game's grammar. **The sanctioned form of depth, if ever wanted: into-the-screen chase set-pieces** (boulder-style, camera behind the robot) as self-contained sector-finale cells — a separate bounded mini-sim, zero new keys, no pollution of solver/predictor/controls. Someday-card, not a next step.
- Build: still a hand-authored single file. When Phase 1 lands (player-model + director modules) or lanes happen, split into modules with a vite single-page build like the other games in this repo — ship artifact stays one HTML file, so nothing in §5 changes.
