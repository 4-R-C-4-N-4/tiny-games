...
- **Semi-3D (decided 2026-07-28): no continuous depth.** Crash-style analog z-movement would be a new engine and — worse — would break the adversarial stack's core assumptions: the deterministic 2D sim, the jump-template solver's search space, and the landing predictor's 1-D landing distribution all live on the x-axis. If we ever want the depth *flavor*, the honest middle is **discrete lanes** (2 planes, tag every entity/cell with a lane, lane-shift only at authored shift pads à la Klonoa/Pandemonium): solver stays finite (state × {0,1}), telemetry gains one categorical column, and it arrives as new cell types rather than an engine rewrite. Defer until after Phase 1 — it doubles the cell-design space, and the director should learn to steer the current vocabulary first.
